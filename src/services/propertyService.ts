import { TProduct,TFilter, FetchProductsResponse, ICreateProperty, IImage } from "../services/d";
...
export const fetchProperties = async (filters: TFilter): Promise<FetchProductsResponse> => {
  ...
  const response = await handleCall(`products?${queryString}`, "GET", undefined, "no-cache");
